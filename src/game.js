  ...
  create() {
    ...
    this.add.text(cx, cy + 25, "Register", { fontSize: "22px", fill: "#0ff" })
      ...
      .on("pointerdown", () => {
        ...
        // ✅ IMPORTANT FIX:
        // promptRegister() keeps the modal open (register -> login inside same modal).
        // So DO NOT await here; instead, redirect when the promise finally resolves (after login).
        window.Ball10Auth.promptRegister()
          .then(async (maybeUser) => {
            // Resolves after the user actually logs in inside the same modal
            if (maybeUser && maybeUser.username) {
              await initUserStateFromDbIfLoggedIn();
              this.scene.start("MainMenu");
              return;
            }
            // If it resolves without a user (rare), just unlock UI
            this.status.setText("Now login.");
            this._busyAuth = false;
          })
          .catch((e) => {
            const msg = e?.message || String(e);
            // Cancelled shouldn't be treated as a failure
            if (msg === "Cancelled") {
              this.status.setText("Choose an option:");
            } else {
