  ...
  create() {
    ...
    this.add.text(cx, cy + 25, "Register", { fontSize: "22px", fill: "#0ff" })
      ...
      .on("pointerdown", async () => {
        ...
        try {
          // ✅ IMPORTANT FIX:
          // auth.js register flow keeps the modal open and only RESOLVES when the user logs in.
          // So we treat the resolved value as "user after login" and redirect correctly.
          const result = await window.Ball10Auth.promptRegister();

          // If the modal resolved with a user (login happened inside the same modal),
          // go to MainMenu immediately.
          ...
          // Otherwise, user probably cancelled or something unexpected happened
